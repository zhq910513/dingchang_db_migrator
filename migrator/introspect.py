...
def _get_columns_and_generated_flags(table_name: str) -> List[Tuple[str, bool]]:
    ...
    MySQL: EXTRA 包含 'GENERATED' / 'VIRTUAL GENERATED' / 'STORED GENERATED'
    """
    cfg = load_config()
    ...
    for r in rows:
        ...
        is_gen = "GENERATED" in extra
        out.append((r["COLUMN_NAME"], is_gen))
    return out
...
def get_writable_columns(table_name: str) -> List[str]:
    """
    可写列：排除 generated columns（否则 INSERT/UPDATE 会报错 3105）
    """
